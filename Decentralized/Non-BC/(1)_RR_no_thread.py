...
def federated_learning(nodes_data, num_epochs, num_fl_rounds):
    try:
        ...
        for fl_round in range(1, num_fl_rounds + 1):
            ...
            for epoch in range(1, num_epochs + 1):
                ...
                for node_id, (X_train, y_train, X_test, y_test) in enumerate(nodes_data):
                    weights = local_training(node_id, X_train, y_train, X_test, y_test, global_model, node_metrics, epoch, fl_round, aggregator_node)
        ...
        columns = ['FL_Round', 'Epoch', 'Node_ID', 'Accuracy', 'Loss', 'FP', 'TP', 'FN', 'TN', 'Epoch_Time', 'Aggregator_Node']
        ...
        # Save to CSV
        node_metrics_df.to_csv('RoundRobin_FedAvg_Final.csv', index=False)
