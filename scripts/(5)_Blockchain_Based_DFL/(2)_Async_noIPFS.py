...
node_data_paths = [
    "../../dataset_partitions/partition_1.csv", 
    "../../dataset_partitions/partition_2.csv", 
    "../../dataset_partitions/partition_3.csv", 
    "../../dataset_partitions/partition_4.csv", 
    "../../dataset_partitions/partition_5.csv"
]
...
# Metrics CSV setup
metrics_path = os.path.join(OUTPUT_DIR, 'Metrics_(2)_Async_FedAvg_noIPFS.csv')
with open(metrics_path, 'w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(["Node", "Epoch", "FL Round", "Accuracy", "Loss", "Epoch Time", "FL Round Time", "Convergence Time", "Confusion Matrix", "Contributing Nodes", "Miner Node"])
...
for FL_round in range(FL_rounds):
            ...
            with open(metrics_path, 'a', newline='') as file:
                ...
                for epoch in range(epochs):
                    writer.writerow([
                        ...
                        convergence_time, 
                        conf_matrix, 
                        "",  # Placeholder for Contributing Nodes
                        ""   # Placeholder for Miner Node
                    ])
    ...
    # Update the metrics CSV with miner node info
    with open(metrics_path, 'r') as read_file:
        rows = list(csv.reader(read_file))
    
    with open(metrics_path, 'w', newline='') as write_file:
        writer = csv.writer(write_file)
        for row in rows:
            if row[2] == str(FL_round + 1) and row[9] == "":  # Check if it's the current FL round and miner node is empty
                row[9] = ", ".join(map(str, contributing_nodes))
                row[10] = miner_node
            writer.writerow(row)
