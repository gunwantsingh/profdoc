...
file_name = "FedMedian_smpc_metrics.csv"
...
def secure_aggregation(weights):
    try:
        ...
        # Perform secure aggregation by computing the median of encrypted shares
...
# Function to perform FedMedian
def fed_median(local_updates):
    try:
        global_model = np.median(local_updates, axis=0)
        return global_model
    except Exception as e:
        print("Error in fed_median:", e)
        return None
...
with open(file_name, mode='w', newline='') as csvfile:
    ...
    for fl_round in range(1, num_fl_rounds + 1):
        ...
        global_model = fed_median(local_updates)
