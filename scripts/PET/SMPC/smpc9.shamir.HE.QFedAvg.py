...
file_name = "QFedAvg_smpc_metrics.csv"
...
def quantize(weights):
    ...
    return weights  # For now, returning the original weights

# Function for Q-FedAvg aggregation
def qfedavg(local_updates, global_model):
    try:
        # Quantize local updates
        quantized_updates = [quantize(update) for update in local_updates]

        # Compute weighted average of quantized updates
        num_updates = len(quantized_updates)
        weights_sum = np.zeros_like(global_model)
        for update in quantized_updates:
            for i in range(len(update)):
                weights_sum[i] += update[i]

        averaged_weights = weights_sum / num_updates
...
with open(file_name, mode='w', newline='') as csvfile:
    ...
    for fl_round in range(1, num_fl_rounds + 1):
        ...
        # Perform Q-FedAvg aggregation
        start_aggregation_time = time.time()
        print("Aggregation happening now...")
        global_model = qfedavg(local_updates, global_model)
