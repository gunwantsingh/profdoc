...
# Federated Average Function with SMPC
def average_encrypted_models_smpc(encrypted_models):
    num_models = len(encrypted_models)
    aggregated_model = np.zeros_like(encrypted_models[0])

    # Perform secure aggregation using SMPC (Secure Sum)
    for i in range(num_models):
        aggregated_model += encrypted_models[i]
...
try:
    ...
    if loaded_dataset is not None:
        ...
        # Simulate five federated clients
        clients = 5
        ...
        for round_num in range(FL_rounds):
            ...
            for epoch_num in range(30):
                ...
                for i in range(clients):
                    ...
                    # Encrypt local model weights (simulating encrypted model updates)
                    local_models.append(local_model_weights)

                # Aggregation with SMPC (Secure Sum)
                start_time = time.time()
                average_model = average_encrypted_models_smpc(local_models)
                end_time = time.time()
...
metrics_df.to_csv('FedAvg_SMPC_metrics.csv', index=False)
