...
def qfedavg_aggregate(local_models):
    # Aggregate using QFedAvg algorithm
    ...
    for i in range(len(global_model_weights)):
        ...
        q = len(local_models)
        q_weights = [1 / q] * q
        avg_update = np.average(updates, axis=0, weights=q_weights)
...
try:
    ...
    if loaded_dataset is not None:
        ...
        for round_num in range(FL_rounds):
            ...
            qfedavg_aggregate(local_models)
        ...
        metrics_df.to_csv('Simply_QFedAvg.csv', index=False)
