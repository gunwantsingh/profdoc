...
def compute_privacy(train_data, num_examples, batch_size, noise_multiplier, epochs):
    # Placeholder function for computing privacy (removed for simplicity)
    return None

def fedadam_aggregate(local_models):
    # Aggregate using FedAdam algorithm
    global_model_weights = global_model_updated.get_weights()

    for i in range(len(global_model_weights)):
        for model_weights in local_models:
            global_model_weights[i] += model_weights[i]

        global_model_weights[i] /= len(local_models)
...
try:
    ...
    if loaded_dataset is not None:
        ...
        for round_num in range(FL_rounds):
            ...
            fedadam_aggregate(local_models)
        ...
        metrics_df.to_csv('Simply_FedAdam.csv', index=False)
