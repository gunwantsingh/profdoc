...
def fedadagrad_aggregate(local_models):
    # Aggregate using FedAdagrad algorithm
    global_model_weights = global_model_updated.get_weights()

    for i in range(len(global_model_weights)):
        gradients = [local_model_weights[i] - global_model_weights[i] for local_model_weights in local_models]
        global_model_weights[i] += np.mean(gradients, axis=0)
...
try:
    ...
    if loaded_dataset is not None:
        ...
        for round_num in range(FL_rounds):
            ...
            fedadagrad_aggregate(local_models)
        ...
        metrics_df.to_csv('Simply_FedAdagrad.csv', index=False)
