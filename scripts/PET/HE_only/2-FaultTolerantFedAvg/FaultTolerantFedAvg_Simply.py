...
epochs = 30  # Number of epochs for local training
...
# Fault-Tolerant Federated Average Function
def fault_tolerant_average_encrypted_models(context, encrypted_models):
    num_models = len(encrypted_models)
    return sum(encrypted_models) if num_models > 0 else np.zeros(input_size)
        ...
        for round_num in range(FL_rounds):
            ...
            for epoch_num in range(epochs):
                ...
                for i in range(clients):
                    ...
                    local_models.append(local_model.get_weights()[0].flatten())

                # Aggregation
                start_time = time.time()
                average_model = fault_tolerant_average_encrypted_models(None, local_models)
                end_time = time.time()
...
metrics_df.to_csv('FaultTolerantFedAvg_Simply_metrics.csv', index=False)
