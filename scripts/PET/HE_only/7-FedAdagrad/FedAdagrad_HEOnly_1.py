...
def homomorphic_model_update(global_model, encrypted_local_model):
    # Perform addition on encrypted vectors
    sum_encrypted_models = global_model + encrypted_local_model
    return sum_encrypted_models

# Decryption Function
def decrypt_result(context, result):
    return result.decrypt()

# Federated Adagrad Function
def fed_adagrad_update(global_model, local_gradients, learning_rate):
    # Initialize global accumulative squared gradients
    if global_model is None:
        global_model = np.zeros(input_size)

    # Update global model using Adagrad
    for gradient in local_gradients:
        global_model += np.square(gradient)

    # Apply learning rate and square root to global model
    global_model_update = learning_rate / np.sqrt(global_model + 1e-8)

    return global_model_update

# Quantization Function
def quantize_model_weights(model_weights):
    return np.round(model_weights * 255).astype(np.uint8)

# Dequantization Function
def dequantize_model_weights(quantized_weights):
    return quantized_weights / 255.0
...
try:
    ...
    if loaded_dataset is not None:
        ...
        global_accumulative_gradients = None  # Initialize global accumulative gradients
        ...
        for round_num in range(FL_rounds):
            ...
            for epoch_num in range(30):
                ...
                local_gradients = []  # Initialize local gradients list
                ...
                for i in range(clients):
                    ...
                    local_gradients.append(local_model_weights)

                # Aggregation - FedAdagrad Accumulative
                start_time = time.time()
                global_accumulative_gradients = fed_adagrad_update(global_accumulative_gradients, local_gradients, learning_rate)
                end_time = time.time()
                aggregation_time = end_time - start_time

                print("\nUpdated global model weights:", global_accumulative_gradients)
...
metrics_df.to_csv('FedAdagrad_HEOnly_metrics.csv', index=False)
