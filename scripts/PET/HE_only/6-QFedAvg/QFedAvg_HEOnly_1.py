...
# Federated Averaging Function with Quantization
def qfed_avg_encrypted_models(context, encrypted_models):
    num_models = len(encrypted_models)
    if num_models > 0:
        # Quantize the encrypted models
        quantized_encrypted_models = [model * 255 for model in encrypted_models]  # Scale by 255 for quantization

        # Compute average of quantized encrypted models
        average_encrypted_model = sum(quantized_encrypted_models) * (1 / num_models)

        return average_encrypted_model
    else:
        return encrypt_data(context, np.zeros(input_size))
    ...
    if loaded_dataset is not None:
        ...
        for round_num in range(FL_rounds):
            ...
            for epoch_num in range(30):
                ...
                average_model = qfed_avg_encrypted_models(context, local_models)
                end_time = time.time()
                aggregation_time = end_time - start_time

                print("\nUpdated global model weights:", average_model)
...
metrics_df.to_csv('QFedAvg_HEOnly_metrics.csv', index=False)
