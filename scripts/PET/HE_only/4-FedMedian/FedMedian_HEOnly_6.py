# Final FedMedian + HE true, no decryption + metrics + HeOnly + noD
#
import numpy as np
import pandas as pd
...
# Fault-Tolerant Federated Median Function
def fault_tolerant_median_encrypted_models(encrypted_models):
    num_models = len(encrypted_models)
    if num_models > 0:
        # Sum all encrypted models
        sum_encrypted_models = sum(encrypted_models)
        # Compute the median
        median_encrypted_model = sum_encrypted_models * (1 / num_models)
        return median_encrypted_model
    else:
    ...
    if loaded_dataset is not None:
        ...
        for round_num in range(FL_rounds):
            ...
            for epoch_num in range(30):
                ...
                median_model = fault_tolerant_median_encrypted_models(local_models)
                global_model_updated = homomorphic_model_update(
                    np.zeros(input_size), median_model
                )
                end_time = time.time()
...
metrics_df.to_csv('FedMedian_HEOnly_metrics.csv', index=False)
