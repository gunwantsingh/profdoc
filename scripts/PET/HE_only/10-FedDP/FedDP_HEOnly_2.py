# This example uses FedDP + Adds noise + HE
#
import numpy as np
import pandas as pd
        ...
        for round_num in range(FL_rounds):
            ...
            for epoch_num in range(30):
                ...
                for i in range(clients):
                    ...
                    local_models.append(encrypt_data(context, local_model_weights_noisy))  # Encrypt the noisy weights

                # Aggregation (in the encrypted domain)
                start_time = time.time()
                global_model_encrypted = sum(local_models)  # Homomorphically sum the encrypted weights
                global_model = global_model_encrypted.decrypt()  # Decrypt the aggregated encrypted weights
                end_time = time.time()
...
metrics_df.to_csv('FedDP_HE_metrics.csv', index=False)
