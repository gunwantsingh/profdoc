...
lambda_value = 0.01  # Proximal term coefficient
...
def homomorphic_model_update(global_model, aggregated_model):
    updated_model = global_model + aggregated_model
    return updated_model

# Federated Average Function with FedProx
def average_models(models, global_model):
    num_models = len(models)

    # Aggregate model weights
    aggregated_model = sum(models)

    # Add FedProx term to the aggregated model
    proximal_term = lambda_value * (aggregated_model - global_model)
    aggregated_model += proximal_term
    ...
    if loaded_dataset is not None:
        ...
        for round_num in range(FL_rounds):
            ...
            for epoch_num in range(30):
                ...
                aggregated_model = average_models(local_models, global_model)
                global_model_updated = homomorphic_model_update(global_model, aggregated_model)
                end_time = time.time()
...
metrics_df.to_csv('FedProx_Simply_metrics.csv', index=False)
