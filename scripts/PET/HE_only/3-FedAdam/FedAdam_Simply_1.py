    ...
    if loaded_dataset is not None:
        ...
        for round_num in range(FL_rounds):
            ...
            for epoch_num in range(30):
                ...
                # Aggregation (FedAdam)
                aggregated_weights = np.mean(local_models, axis=0)
                global_model_weights = np.zeros_like(aggregated_weights)
                global_model_weights += aggregated_weights

                # Update epoch-level metrics
                round_metrics['Server Aggregation Time (seconds)'] = 0
...
metrics_df.to_csv('FedAdam_Simply_metrics.csv', index=False)
