# FedAdam + HE + noDecryption on Server + metrics
#
# Here is the difference between FedAvg and FedAdam in python script:
# FedAvg is a federated optimization algorithm that updates the global model on the server by averaging the local model updates from the clients.
# FedAdam is an adaptive federated optimization algorithm that updates the global model on the server by averaging the local model updates from the clients, and also uses the Adam optimizer to update the local models on the clients.

# 05 March: Changed Dataset to BotNetIoT and it is working!!!!
# Now I want to compare privacy guarantees + robustness to attacks -> Evasion attack is most common.
#
#
import numpy as np
import pandas as pd
...
# Fault-Tolerant Federated Average Function THIS IS FEDADAM.
def fault_tolerant_average_encrypted_models(context, encrypted_models):
    num_models = len(encrypted_models)
    return sum(encrypted_models) if num_models > 0 else encrypt_data(context, np.zeros(input_size))
    ...
    if loaded_dataset is not None:
        ...
        for round_num in range(FL_rounds):
            ...
            for epoch_num in range(30):
                ...
                average_model = fault_tolerant_average_encrypted_models(context, local_models)
...
metrics_df.to_csv('FedAdam_HEOnly_metrics.csv', index=False)
