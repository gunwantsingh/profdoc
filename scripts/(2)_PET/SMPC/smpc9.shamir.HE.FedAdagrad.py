...
file_name = "FedAdagrad_smpc_metrics.csv"
...
def secure_aggregation(weights):
    try:
        ...
        encrypted_shares = []
        for share in shares:
            encrypted_shares.append([ckks_encoder.encode(val) for val in share])
...
# Function for adaptive learning rate adjustments in FedAdagrad
def adjust_learning_rate(learning_rate, gradient_squared, global_model):
    # Placeholder for adaptive learning rate adjustments
    return learning_rate  # For now, returning the original learning rate

# Function for FedAdagrad aggregation
def fedadagrad(local_updates, global_model, learning_rate):
    try:
        # Placeholder for FedAdagrad aggregation logic
        adjusted_learning_rate = adjust_learning_rate(learning_rate, gradient_squared, global_model)
        return updated_model_weights  # For now, returning the original global model weights
    except Exception as e:
        print("Error:", e)
        return None  # Return None or handle the error as appropriate
...
with open(file_name, mode='w', newline='') as csvfile:
    ...
    for fl_round in range(1, num_fl_rounds + 1):
        ...
        # Perform FedAdagrad aggregation
        start_aggregation_time = time.time()
        print("Aggregation happening now...")
        global_model = fedadagrad(local_updates, global_model, learning_rate)
