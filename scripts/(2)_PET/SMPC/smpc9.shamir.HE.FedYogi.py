...
# Function for Shamir's Secret Sharing
...
# Function for secure aggregation using TenSEAL
def secure_aggregation(weights):
    try:
        ...
        # Convert shares to encrypted shares using TenSEAL
        context = ts.Context(ts.SCHEME_TYPE.CKKS, poly_modulus_degree=8192, coeff_mod_bit_sizes=[60, 40, 40, 60])
        context.global_scale = 2**40
        ckks_encoder = ts.ckks_ckks_encoder(context)

        encrypted_shares = [[ckks_encoder.encode(pair) for pair in share] for share in shares]

        # Perform secure aggregation by adding encrypted shares
        aggregated_result = encrypted_shares[0].copy()
        for i in range(1, num_weights):
            for j in range(len(aggregated_result)):
                aggregated_result[j] += encrypted_shares[i][j]
...
# Function for FedYogi aggregation
def fed_yogi(local_updates, global_model, learning_rate):
    try:
        # Update global model using adaptive learning rate
        for i, local_update in enumerate(local_updates):
            for layer_index in range(len(global_model)):
                # Apply adaptive learning rate adjustment for each layer
                global_model[layer_index] -= learning_rate * local_update[layer_index]  # Update with adaptive learning rate
...
with open(file_name, mode='w', newline='') as csvfile:
    ...
    for fl_round in range(1, num_fl_rounds + 1):
        ...
        # Apply FedYogi aggregation
        global_model = aggregated_result  # Initialize global model with aggregated result
        global_model = fed_yogi(local_updates, global_model, learning_rate)
