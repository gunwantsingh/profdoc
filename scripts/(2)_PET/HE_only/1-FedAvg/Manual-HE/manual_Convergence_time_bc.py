...
fl_round_time_columns = [col for col in df.columns if 'Convergence Time' in col]
...
if fl_round_time_columns:
    ...
    plt.title('FL Round vs. FL Convergence Time (PET)')
    plt.xlabel('FL Round')  # Assuming the index represents sequential rounds
    plt.ylabel('FL Convergence Time - PET (seconds)')
    plt.legend()
    plt.grid(True)
    ...
    print(f"No columns with 'FL Convergence Time' found in {csv_file}.")
