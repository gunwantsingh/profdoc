    ...
    if loaded_dataset is not None:
        ...
        for round_num in range(FL_rounds):
            ...
            for epoch_num in range(30):
                ...
                if global_accumulative_gradients is None:
                    global_accumulative_gradients = np.zeros_like(local_gradients[0])
                for gradient in local_gradients:
                    global_accumulative_gradients += np.square(gradient)
                end_time = time.time()
...
metrics_df.to_csv('FedAdagrad_Simply_metrics.csv', index=False)
