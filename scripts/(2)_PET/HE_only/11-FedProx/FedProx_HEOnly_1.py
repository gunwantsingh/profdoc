...
def average_encrypted_models(context, encrypted_models, global_model):
    ...
    proximal_term = lambda_value * (aggregated_model - encrypt_data(context, global_model))
    ...
    if loaded_dataset is not None:
        ...
        for round_num in range(FL_rounds):
            ...
            for epoch_num in range(30):
                ...
                average_model = average_encrypted_models(context, local_models, np.zeros(input_size))
...
metrics_df.to_csv('FedProx_HEOnly_metrics.csv', index=False)
