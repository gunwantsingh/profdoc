...
def fedmedian_aggregate(local_models):
    # Aggregate using FedMedian algorithm
    global_model_weights = global_model_updated.get_weights()

    for i in range(len(global_model_weights)):
        weights = [model_weights[i] for model_weights in local_models]
        global_model_weights[i] = np.median(weights, axis=0)
...
try:
    ...
    if loaded_dataset is not None:
        ...
        for round_num in range(FL_rounds):
            ...
            fedmedian_aggregate(local_models)
        ...
        metrics_df.to_csv('Simply_FedMedian.csv', index=False)
