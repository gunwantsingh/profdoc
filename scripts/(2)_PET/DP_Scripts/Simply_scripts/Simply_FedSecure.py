...
def create_model():
    ...
    # Switch to SGD optimizer for FedSecure
...
def fedsecure_aggregate(local_models):
    # Aggregate using FedSecure algorithm
    global_model_weights = global_model_updated.get_weights()

    for i in range(len(global_model_weights)):
        local_weights = [local_model_weights[i] for local_model_weights in local_models]
        global_model_weights[i] = np.mean(local_weights, axis=0)
...
try:
    ...
    if loaded_dataset is not None:
        ...
        for round_num in range(FL_rounds):
            ...
            fedsecure_aggregate(local_models)
        ...
        metrics_df.to_csv('Simply_FedSecure.csv', index=False)
